...
# This script reproduces results from Table 3 in the paper
# It evaluates ablation of past-reconstruction vs next-event-prediction
# tasks from training of the embedding model.
...
do
    for METHOD in "np_ne" "np_only" "ne_only"
    do
        if [[ $METHOD = "np_ne" ]]
        then
        ...
        export OPTUNA_STUDY_NAME="ablations_${DATASET}_${METHOD}"
        export CACHE=$SCRIPT_DIR/caches/ablations/${DATASET}_${METHOD}.pickle
